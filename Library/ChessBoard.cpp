...
void ChessBoard::printBoard() {
	...
	ChessPiece* piece;
	BOOLBOARD validMoveBoard = { false };
	...
	// Get valid moves from selected piece
	if (selectedPiece != nullptr) {
		selectedPiece->getAvailableMoves(&validMoveBoard);
	}
		...
		for (int x = A; x <= H; x = x++) {
			...
			if (piece != nullptr) {
				if (piece == selectedPiece) {
					...
					SetConsoleTextAttribute(console_color, (validMoveBoard.board[x][y] == true ? 41 : 9)); // Blue
				}
				else {
					SetConsoleTextAttribute(console_color, (validMoveBoard.board[x][y] == true ? 47 : 15)); // White
				}
